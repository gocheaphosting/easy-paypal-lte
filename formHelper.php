    ...
    function loadRowSetFromDB($rowSetName, &$rowSet, $meta=false) {
      ...
      foreach ($rowSet as $k => $v) {
        ...
        if (!empty($associatedKey)) {
          ...
          switch($type){
          case 'button':
            $rowSet[$k]['inline'] = sprintf("<input type='%s' value='Show' onmouseover=\"Tip('%s', WIDTH, 250, TITLE, '%s', FIX, [this, -10, 5])\" onmouseout=\"UnTip()\" onclick=\"window.prompt('Copy to clipboard: Ctrl/Cmd-C:', '%s')\">", $type, "Hidden value associated with this option is:<br /><code>$associatedKey => $associatedVal</code><br /><b><em>Click to reveal and copy to clipboard.</em></b><br />", "Associated Value", $associatedVal) ;
            break ;
          case 'text':
