...
if (class_exists("FormHelper")) {
  ...
  class FormHelper {
    ...
    function isValid($control, $s) {
      ...
      $validator = 'validate_';
      if (!empty($control['validator'])) {
        $validator .= $control['validator'];
      }
      if (method_exists($this, $validator)) {
        $err .= $this->$validator($s);
      }
      if (!empty($err)) {
        return $err;
      }
      ...
      switch ($type) {
        ...
        default :
          ...
          if (!empty($control['unique']) and $control['unique']) {
            $db = $this->db;
            if (!empty($control['table'])) {
              $table = $control['table'];
            }
            if (!empty($control['column'])) {
    ...
    function showOneRow($k, $v) {
      ...
      $selectLike = false;
      $state = '';
      if (!empty($v['disabled']) && $v['disabled']) {
        $state = ' disabled="dsiabled"';
      }
      $o = '';
      $inline = '';
      if (!empty($v['inline'])) {
      ...
      switch ($type) {
        ...
        case 'dbeditableselect' :
          if (!empty($v['table']) && !empty($v['column'])) {
            ...
            break;
          } // only if table and column are not given
        case 'editableselect' :
          if (!empty($v['form']) && !empty($v['options'])) {
            ...
            if (!empty($v['unique']) && $v['unique']) {
              $o .= sprintf(" onchange='document.%s.%s_autoload.value=1;document.%s.submit();'", $v['form'], $v['form'], $v['form']);
              /* $browse = sprintf("<input class='button' type='submit' value='...' " .
                "title='Load the information from the Database' name='%s_load'>",
                $v['form']) ; */
              $browse = sprintf("<input type='hidden' name='%s_autoload'>", $v['form']);
            }
            else {
    ...
    function printFormTableHeader($title, $subtitle, $errMsg = '', $class = "setup", $openRow = "<tr><td colspan=3>") {
    ...
    function renderForm($formName, $rowSet, $title, $subtitle, $buttonText, $errMsg = '', $renderFormStart = true) {
      ...
      if ($renderFormStart && !empty($formName)) {
        printf('    <form action="%s" method="post" name="%s" %s>', $script, $formName, $enc);
      }
    ...
    function renderDoubleForm($formName, $rowSet1, $title1, $subtitle1, $rowSet2, $title2, $subtitle2, $buttonText, $errMsg = '') {
    ...
    function loadRowSetFromDB($rowSetName, &$rowSet, $meta = false) {
      ...
      if (!empty($_POST[$rowSetName . '_load'])) {
        return;
      }
      if (!empty($_POST[$rowSetName . '_autoload'])) {
        return;
      }
      $db = $this->db;
      if ($meta) {
        $rowSetVals = $db->getMetaData($rowSetName);
      }
      else {
        $rowSetVals = $db->getRowData($rowSetName);
      }
      foreach ($rowSet as $k => $v) {
        if (!is_array($v)) {
          break;
        }
        if (isset($rowSetVals[$k])) {
          $rowSet[$k]['value'] = $rowSetVals[$k];
        }
        $associatedKey = '';
        if (!empty($rowSet[$k]['hidden'])) {
          $associatedKey = $rowSet[$k]['hidden'];
        }
        if (!empty($associatedKey)) {
          ...
          if (!$rowSet[$k]['value']) {
            ...
            if (empty($rowSet[$k]['reveal'])) {
              $type = '';
            }
            else {
              $type = strtolower($rowSet[$k]['reveal']);
            }
          }
          ...
          // to be tested
          $rowSet[$associatedKey]['type'] = 'ignore';
        }
      }
    }
    ...
    static function mkStorageName($storage) {
      if (function_exists('wp_upload_dir')) {
        if (strpbrk($storage, "/.\\:") === false) {
          ...
          $to = "{$upload['basedir']}/$storage";
          $from = dirname(__FILE__);
          $storage = self::getRelativePath($from, $to);
        }
      }
      return $storage;
    }
    ...
    function handleSubmit($formName, &$rowSet, $meta = false) {
      ...
      if (!empty($_POST[$formName . '-submit'])) {
        ...
        if (!empty($rowSet['file'])) {
          ...
          $prod = $db->getData("products", "file", array('product_code' => $dbRow['product_code']));
          if (!empty($prod)) {
            $fileName = $prod[0]['file'];
          }
          else {
            $fileName = '';
          }
        ...
        if ($this->isRowSetValid($rowSet)) {
          if ($meta) {
            $db->putMetaData($formName, $dbRow);
          }
          else {
            $db->putRowData($formName, $dbRow);
          }
